...
Linux.TransparencyEnabled = false
Linux.TransparencyValue = 0.3
Linux.Themes = {
...
function Linux.RegisterTransparencyElement(element)
    table.insert(Linux.TransparencyElements, element)
end
...
function Linux.ApplyTransparency(enabled, value)
    Linux.TransparencyEnabled = enabled
    if enabled then
        Linux.TransparencyValue = value or 0.3
    end
    
    for _, element in pairs(Linux.TransparencyElements) do
        if element and element.Parent and element.Name ~= "Section" then
            local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local targetTransparency = enabled and Linux.TransparencyValue or (element:GetAttribute("OriginalTransparency") or 0.8)
            TweenService:Create(element, tweenInfo, {BackgroundTransparency = targetTransparency}):Play()
        end
    end
end
...
function Linux.SaveTransparencySettings()
    local success, result = pcall(function()
        local transparencyData = {
            Enabled = Linux.TransparencyEnabled,
            Value = Linux.TransparencyValue
        }
        local json = HttpService:JSONEncode(transparencyData)
        if writefile then
            if not isfolder(Linux.ConfigFolder) then
                makefolder(Linux.ConfigFolder)
            end
            writefile(Linux.ConfigFolder .. "/transparency.json", json)
            return true
        else
            return false, "writefile function not available"
        end
    ...
    if not success then
        print("Error saving transparency settings:", result)
    end
    return success, result
end

function Linux.LoadTransparencySettings()
    local success, result = pcall(function()
        if readfile and isfile(Linux.ConfigFolder .. "/transparency.json") then
            local json = readfile(Linux.ConfigFolder .. "/transparency.json")
            local transparencyData = HttpService:JSONDecode(json)
            Linux.TransparencyEnabled = transparencyData.Enabled or false
            Linux.TransparencyValue = transparencyData.Value or 0.3
            return true
        else
            return false, "Transparency settings file not found"
        end
    end)
    if not success then
        print("Error loading transparency settings:", result)
    end
    return success, result
end
    ...
    local success, result = pcall(function()
        if listfiles and isfolder(Linux.ConfigFolder) then
            ...
            for _, file in pairs(files) do
                ...
                if configName and configName ~= "autoload" and configName ~= "privacy" and configName ~= "theme" and configName ~= "transparency" then
...
function Linux.Create(config)
    ...
    Linux.LoadTransparencySettings()
    ...
    if config.Subtitle then
        local function formatSubtitle(text)
            if string.len(text) > 25 then
                ...
                for _, word in ipairs(words) do
                    if string.len(currentLine .. " " .. word) <= 25 then
                        if currentLine == "" then
                            currentLine = word
                        else
        ...
        SubtitleLabel = Linux.Instance("TextLabel", {
            ...
            BackgroundTransparency = 1,
            Size = UDim2.new(0, 0, 1, 0),
            Position = UDim2.new(0, TitleLabel.AbsoluteSize.X + titleOffset + 8, 0, -2),
            Font = Enum.Font.GothamSemibold,
            Text = formatSubtitle(config.Subtitle),
        ...
        TitleLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            SubtitleLabel.Position = UDim2.new(0, TitleLabel.AbsoluteSize.X + titleOffset + 8, 0, -2)
        end)
    end
    ...
    function LinuxLib.Tab(config)
        ...
        -- REMOVIDO: Linux.RegisterTransparencyElement(TabBtn)
        -- Isso corrige o problema de transparência nas abas
        ...
        local function SelectTab()
            ...
            TabBtn.BackgroundTransparency = Linux.TransparencyEnabled and Linux.TransparencyValue or 0.8
        ...
        function TabElements.Button(config)
            ...
            local frameHeight = hasDescription and 58 or 36
            ...
            BtnFrame:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(BtnFrame)
            ...
            local Btn = Linux.Instance("TextButton", {
                Parent = BtnFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, hasDescription and 0.6 or 1, 0),
                Position = UDim2.new(0, 0, 0, 0),
                Font = Enum.Font.GothamSemibold,
                Text = config.Name,
                TextColor3 = Color3.fromRGB(200, 200, 210),
            ...
            if hasDescription then
                Linux.Instance("TextLabel", {
                    ...
                    Position = UDim2.new(0, 10, 0.6, -5), -- Movido mais próximo do título
                    Font = Enum.Font.GothamSemibold,
            ...
            Btn.MouseLeave:Connect(function()
                local targetTransparency = Linux.TransparencyEnabled and Linux.TransparencyValue or 0.7
                TweenService:Create(BtnFrame, TweenInfo.new(0.2), {BackgroundTransparency = targetTransparency}):Play()
            end)
        ...
        function TabElements.Toggle(config)
            ...
            Toggle:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(Toggle)
            ...
            local ToggleText = Linux.Instance("TextLabel", {
                Parent = Toggle,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, -60, hasDescription and 0.4 or 1, 0),
            ...
            if hasDescription then
                Linux.Instance("TextLabel", {
                    Parent = Toggle,
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, -70, 0.6, 0),
                    Position = UDim2.new(0, 10, 0.4, -5), -- Movido mais próximo do título
                    Font = Enum.Font.GothamSemibold,
                    Text = config.Description,
                    TextColor3 = Color3.fromRGB(150, 150, 160),
                    TextSize = 12,
        ...
        function TabElements.Dropdown(config)
            ...
            Dropdown:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(Dropdown)
            ...
            Linux.Instance("TextLabel", {
                ...
                BackgroundTransparency = 1,
                Size = UDim2.new(0.6, 0, hasDescription and 0.4 or 1, 0),
                Position = UDim2.new(0, 10, 0, 0),
                Font = Enum.Font.GothamSemibold,
                Text = config.Name,
                TextColor3 = Color3.fromRGB(200, 200, 210),
            ...
            local Selected = Linux.Instance("TextLabel", {
                Parent = DropdownButton,
                BackgroundTransparency = 1,
                Size = UDim2.new(0.3, -21, hasDescription and 0.4 or 1, 0),
            ...
            DropFrame:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(DropFrame)
            ...
            OptionsHolder:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(OptionsHolder)
        ...
        function TabElements.Slider(config)
            ...
            Slider:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(Slider)
            ...
            if hasDescription then
                Linux.Instance("TextLabel", {
                    ...
                    Position = UDim2.new(0, 10, 0, 20), -- Movido mais próximo do título
                    Font = Enum.Font.GothamSemibold,
                    Text = config.Description,
                    TextColor3 = Color3.fromRGB(150, 150, 160),
                    TextSize = 12,
            ...
            local SliderBar = Linux.Instance("Frame", {
                ...
                Position = UDim2.new(0, 10, 0, hasDescription and 48 or 26),
                ZIndex = 2,
                BorderSizePixel = 0,
                BorderColor3 = Color3.fromRGB(39, 39, 42),
                Name = "Bar"
        ...
        function TabElements.Input(config)
            ...
            Input:SetAttribute("OriginalTransparency", 0.7)
            Linux.RegisterTransparencyElement(Input)
            ...
            Linux.Instance("TextLabel", {
                Parent = Input,
                BackgroundTransparency = 1,
                Size = UDim2.new(0.5, 0, hasDescription and 0.4 or 1, 0),
                Position = UDim2.new(0, 10, 0, 0),
                Font = Enum.Font.GothamSemibold,
                Text = config.Name,
                TextColor3 = Color3.fromRGB(200, 200, 210),
        ...
        function TabElements.Label(config)
            ...
            LabelFrame:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(LabelFrame)
        ...
        function TabElements.Paragraph(config)
            ...
            ParagraphFrame:SetAttribute("OriginalTransparency", 0.8)
            Linux.RegisterTransparencyElement(ParagraphFrame)
    ...
    if config.ConfigSave ~= false then
        ...
        local transparencyToggle = SettingsTab.Toggle({
            Name = "Transparency",
            Description = "",
            Default = Linux.TransparencyEnabled,
            Callback = function(state)
                Linux.TransparencyEnabled = state
                Linux.ApplyTransparency(state, Linux.TransparencyValue)
                Linux.SaveTransparencySettings()
                
                -- ADICIONADO: Re-seleciona a aba atual para aplicar transparência corretamente
                if CurrentTab and Tabs[CurrentTab] then
                    local currentTabIndex = CurrentTab
                    CurrentTab = nil
                    local selectTabFunction = Tabs[currentTabIndex].Button.MouseButton1Click
                    if selectTabFunction then
                        selectTabFunction:Fire()
                    end
                end
            end
        })
        ...
        local transparencySlider = SettingsTab.Slider({
            ...
            Min = 0.1,
            Max = 0.9,
            Default = Linux.TransparencyValue,
            Rounding = 1,
            Callback = function(value)
                Linux.TransparencyValue = value
                if Linux.TransparencyEnabled then
                    Linux.ApplyTransparency(true, value)
                    
                    -- ADICIONADO: Re-seleciona a aba atual para aplicar nova transparência
                    if CurrentTab and Tabs[CurrentTab] then
                        local currentTabIndex = CurrentTab
                        CurrentTab = nil
                        local selectTabFunction = Tabs[currentTabIndex].Button.MouseButton1Click
    ...
    task.spawn(function()
        task.wait(1)
        if Linux.TransparencyEnabled then
            Linux.ApplyTransparency(true, Linux.TransparencyValue)
        end
    end)
